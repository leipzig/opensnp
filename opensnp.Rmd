...
read.csv("rawdata/phenotypes_201602180000.csv",sep = ';') %>% distinct -> phenotypes
...
knitr::kable(user_lactose %>% group_by(LactoseScrubbed) %>% dplyr::summarize(count=n()))
```
### Genotypes
```{r}
...
#this looks awful, lets do the math and display a unified table
...
lactosect$expected<-round(p_tot[lactosect$LactoseScrubbed]/sum(p_tot) * p_rs4988235[lactosect$rs4988235]/sum(p_rs4988235) * p_rs182549[lactosect$rs182549]/sum(p_rs182549) * margin.table(al_ct),2)
lactosect$prop<-round(lactosect$observed/lactosect$expected,2)
